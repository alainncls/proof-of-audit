...
const App = () => {
    ...
    const validateField = (name: string, value: string) => {
        switch (name) {
            ...
            case 'repoUrl':
                return /^https:\/\/github\.com\/[^/]+\/[^/]+$/.test(value) ? '' : 'GitHub repo URL is not valid.';
    ...
    const isError = useMemo(() => Object.values(formState.errors).some((error) => error !== ''), [formState.errors]);
    const isEmpty = useMemo(() => Object.values(formState.inputValues).some((value) => value === ''), [formState.inputValues]);

    const truncateHexString = (hexString: string) => `${hexString.slice(0, 7)}...${hexString.slice(hexString.length - 5)}`;
        ...
        <>
            ...
            <div className="main-container">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    />
                    {formState.errors.repoUrl && <div className="error">{formState.errors.repoUrl}</div>}
                    <input
                        type="text"
                        name="commitHash"
                    ...
                    />
                    {formState.errors.commitHash && <div className="error">{formState.errors.commitHash}</div>}
                    <input
                        type="text"
                    ...
                    <button type="submit" disabled={!address || !veraxSdk || isError || isEmpty}>
                        Issue attestation
                    </button>
                </form>
                {txHash && (
