...
- Confirm no mutable refs remain — every external `uses:` must be a 40-character SHA with a version comment:
  ```bash
  grep -rnE 'uses:\s+[^./].*@[^ ]+' .github/workflows .github/actions | grep -vE '@[0-9a-f]{40} +# '
  ```
  The command should print nothing. Any line it prints is an unpinned or uncommented action.
